...
public class AStar : MonoBehaviour
{
    ...
    public Vector2 Compute(Vector2Int start, Vector2Int goal)
    {
        ...
        Actor actor = GameManager.init.GetActorAtLocation(transform.position + (Vector3)stepDirection);
    ...
    private List<Node> FindNeighbours(Vector2Int parentPosition, Vector2Int start)
    {
        ...
        for (int x = -1; x <= 1; x++)
        {
            for (int y = -1; y <= 1; y++)
            {
                ...
                if (y != 0 || x != 0)
                {
                    bool hasFloorOrDoor = MapManager.init.getFloorMap.GetTile((Vector3Int)neighbourPos) || MapManager.init.getInteractableMap.GetTile((Vector3Int)neighbourPos);
    ...
    private int DetermineGScore(Vector2Int neighbour, Vector2Int current)
    {
        ...
        Actor actor = GameManager.init.GetActorAtLocation((Vector3Int)neighbour);
    ...
    {
        if (MapManager.init.getNodes.ContainsKey(position))
        {
            return MapManager.init.getNodes[position];
        }
        else
        {
            Node node = new Node(position);
            MapManager.init.getNodes.Add(position, node);
            return node;
        }
    }
