...
public class AStar : MonoBehaviour {
  ...
  public Vector2 Compute(Vector2Int start, Vector2Int goal) {
    ...
    if (GameManager.init.GetBlockingActorAtLocation(transform.position + (Vector3)stepDirection)) {
      return Vector2.zero;
    }
  ...
  private List<Node> FindNeighbours(Vector2Int parentPosition, Vector2Int start) {
    ...
    for (int x = -1; x <= 1; x++) {
      for (int y = -1; y <= 1; y++) {
        ...
        if (y != 0 || x != 0) {
          if (neighbourPos != start && MapManager.init.getFloorMap.GetTile((Vector3Int)neighbourPos)) {
  ...
  private int DetermineGScore(Vector2Int neighbour, Vector2Int current) {
    ...
    Actor actor = GameManager.init.GetBlockingActorAtLocation((Vector3Int)neighbour);
